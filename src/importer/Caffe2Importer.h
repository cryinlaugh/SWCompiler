...
private:
    IRGraph<float> *graph_;
    std::unordered_map<std::string, IRNode*> name_irnode_map_;
    std::unordered_map<std::string, TensorNode<float>*> name_tNode_map_;
    std::unordered_map<std::string, OpNode<float>*> name_opNode_map_;
...
public:
    Caffe2Importer(IRGraph<float> *g, const std::string &netProtoFile,
                 const std::string &tensorProtoFile, std::vector<TensorNode<float>*> &udef_nodes);
};
}
#endif
