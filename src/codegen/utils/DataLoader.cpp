...
DataLoader::DataLoader(std::string& filename, BytesProto label_proto, BytesProto data_proto, size_t epochs, size_t sample_num, const std::initializer_list<size_t>& label_shape, const std::initializer_list<size_t>& data_shape) {
...
void DataLoader::read(size_t num) {
    ...
    for(size_t i=0; i<num; i++) {
        ...
        switch(_data_bytes_proto) {
            case FOUR_BYTES_AS_FLOAT:
                _stream.read(reinterpret_cast<char*>(_data_buf+i*_data_size), _data_bytes);
                break;
            case ONE_BYTE_AS_INT:
                for(size_t j=0; j<_data_size; j++){
                    _stream.get(c);
                    _data_buf[i*_data_size + j] = static_cast<float>(static_cast<int>(c)); 
                }
            default:
                break;
        }
...
bool DataLoader::next(int *label_batch, float *data_batch) {
    ...
    while(batch_res > 0) {
        ...
        */
        ...
        _data_buf += actual_read_cnt * _data_size; // cnt * element_num_per_sample
