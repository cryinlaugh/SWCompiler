      ...
      for(size_t co=g*oCG; co<(g+1)*oCG; co++) {
        ...
        for(size_t kx=0; kx<kernel_h; kx++){
          for(size_t ky=0; ky<kernel_w; ky++){
            ...
            // filter idx
            size_t fIdx_base = getIdx4d(fdims, co, kx, ky, g*inCG);

            // for each output (x, y)
            for(size_t ox=0; ox<odims[1]; ox++){
              for(size_t oy=0; oy<odims[2]; oy++){
            
                ssize_t ix = (ssize_t)ox*stride_h - pad_t + kx;
                ssize_t iy = (ssize_t)oy*stride_w - pad_l + ky;
                ...
                // input idx
                size_t iIdx_base = getIdx4d(idims, n, ix, iy, g*inCG);
                // output idx
                size_t oIdx = getIdx4d(odims, n, ox, oy, co);

                // for each input channel per group
                for(size_t ci=0; ci<inCG; ci++){
                  output[oIdx] += input[iIdx_base+ci] * filter[fIdx_base+ci];
                } //W
              } // H
            }
          }
    ...
    for(idx[1]=0; idx[1]<idims[1]; idx[1]++)
      for(idx[2]=0; idx[2]<idims[2]; idx[2]++)
        for(idx[3]=0; idx[3]<idims[3]; idx[3]++)
          output[getIdx4d(odims, idx[shuffle[0]], idx[shuffle[1]], idx[shuffle[2]], idx[shuffle[3]])] 
