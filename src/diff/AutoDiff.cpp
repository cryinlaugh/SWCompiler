    ...
    for(auto it=topo_nodes.rbegin(), e=topo_nodes.rend(); it!=e; it++){
        ...
        if(irnode->nodeType() == TENSOR_NODE){
            ...
            if(gradNodeMap.count(node)){
                ...
                auto *sgdOp = new SGDOp(profile.lr, profile.decay, profile.momentum, profile.batch);
            ...
            if(node->childNum() == 0){
                ...
                /*
                auto *opNode = new OpNode(node->name()+"_negloss", new MatrixLogNegLossOp());
                opNode->exlinkUpperNode(node);
                SWLOG_INFO << node->name() << " childNum = " << node->childNum() << "\n";
                net->pushOpNode(opNode);
                */

                auto *tensor = ((TensorNode*)node)->getTensor();
                auto *N = new TensorNode(node->name()+"_grad", new Tensor(tensor->getTensorShape()));
            ...
            if(auto op = dynamic_cast<MatrixMatrixFCOp*>(node->getOp())){
                ...
                auto *N = new OpNode(node->name() + "_grad", new MatrixMatrixFCGradOp());
                ...
                auto *N = new OpNode(node->name() + "_grad", new MatrixTanhGradOp());
                N->exlinkUpperNode(input, output, outputGrad);

                gradNodeMap[node] = N;
                net->pushOpNode(N);
            }
            else if(auto op = dynamic_cast<MatrixSoftmaxOp*>(node->getOp())){
                SWLOG_INFO << "get Gradient node for op " << node->name() << "\n";
                auto *input = node->getParentNode(0);
                ...
                auto *N = new OpNode(node->name() + "_grad", new MatrixSoftmaxGradOp());
            ...
            for(int i=0; i<node->parentNum(); i++){
                ...
                auto *N = new TensorNode(tnode->name()+"_grad", new Tensor(tensor->getTensorShape()), gradNodeMap[node]);

                SWLOG_INFO << "get Gradient node for " << node->name() << " input" << tnode->name() << "\n";
