...
#include "SWC.h"
#include "importer/ONNXImporter.h"
#include <iostream>
...
int main() {
    ...
    auto *data = new TensorNode("data_0", {8, 28, 28, 1});
    data->getTensor()->setTensorInit(TensorInitType::FILE,
                                     "input/mnist_images_8.bin");
    udef.push_back(data);
    ONNXImporter importer(graph, "./mnist.onnx",udef);

    //---------------------------------------------------------------
    auto softmax_t = graph->getNodeByName("softmax_1");
